...
if($id_parcela > $total){
  echo "<script language='javascript'>window.location='aviso_vencido_emprestimo.php';</script>";
}else{
...
if(mysqli_num_rows($sql_parcela) == ''){
	...
	while($res_parcela = mysqli_fetch_array($sql_parcela)){
		$code_vencimento = $res_parcela['vencimento'];
...
$mail->FromName = "A PARCELA $parcela DO SEU CRÉDITO VENCE HOJE, SEGUE BOLETO!";
...
<table width='800' align='center' style='border:20px solid #09C; border-radius:50px; padding:10px;' border='0'>
  ...
  <tr>
    ...
    <br><br>
    Este e-mail é para avisar que a parcela: $parcela de seu crédito pessoal vence HOJE. Para sua comodidade estamos enviando em anexo o boleto de pagamento.</strong></h1>
...
$mail->Subject = "A PARCELA $parcela DO SEU CRÉDITO VENCE HOJE, SEGUE BOLETO!";
